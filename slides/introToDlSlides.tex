...
\begin{document}
\title{Intro to Deep Learning with Keras}   
...
\item Text classification with sequence models
\end{enumerate}
}
...
Goals:
\begin{itemize} 
\item Give you nodding familiarity with theory and practice
\item Give you ample pointers to dive deeper on your own
\item Leave you a starter code-base
\end{itemize}
\bigskip
This class lightly assumes:
\begin{itemize}
\item you have some familiarity with Python
\item knowledge of some general data science concepts
\end{itemize}
}


\frame{
\frametitle{Models Are Function Approximators}

Image here with various functions to approximate a...function

Most estimation is loss minimization.
}

\frame{
\frametitle{Neurons}
Little sigmoid functions...
Equation
Image
}
...
Input is a vector...ex.
Every element goes into a neuron...\\
And the outputs of those neurons go to every neuron in the next layer\\
And then at the end you have a (possibly special) output layer that maps the outputs of the preceding layer to your target values.
...
Light wrapper over TensorFlow (or the other one).\\
(website)
APIs allow simplicity and still power.\\
Great for *using* neural networks, even very complex ones.\\
Terrible for research *on* neural networks.
}

\frame{
\frametitle{Setup}
pip install keras
 -- it brings along (stuff you'll likely need)
(that was easy)

Setup with/for GPU-based estimation:
How?
Why?
...
\frame{
\frametitle{When Would You Use This?}
Um...
}

\frame{
\frametitle{Image Classification with Neural Networks}
Images are arrays (Height x Width x Depth)
You could unwrap one, and then use a fully connected feed forward network...
But:
Tons of parameters
Portions of images might not be in sample place
}
...
Idea:
Estimation: 
Find some little 3x3xDepth mini-pictures that express the ideas in all your images
Use the same set of mini-pictures everywhere (parameter sharing)
Inference: 
Figure out which mini-picture best approximates the neighborhood of each pixel.
Capture that information, and send it to a dense layer.

}

\frame{
\frametitle{CNNs -- a bit further}
Idea:
Estimation: 
Find some little 3x3xDepth mini-pictures that express the ideas in all your images
Use the same set of mini-pictures everywhere (parameter sharing)

At every OTHER pixel, record the maximum activation in the neighborhood. Drop the rest.
Then, find ANOTHER set of mini-pictures (now each one represents a 6x6 area)
... repeat.

Hierarchy! (strained analogy to visual cortex -- useless).

}

\frame{
\frametitle{CNNs: Visualized}
pointer to some website


}

\frame{
\frametitle{Are Your Images Special?}
Chances are, your images share low-level features with others.
So, get a pre-trained model for everyday objects, cut some layers off, and train your own.
This is called "fine tuning."
\medskip
Unless your images, at a very low level, are really atypical.

}

\frame{
\frametitle{Let's Code One Together!}
